...
const headerPatchMiddleware = {
    pre: async (requestContext) => {
        ...
        requestContext.setHeaderParam('Content-Type', 'application/strategic-merge-patch+json');
...
class K8SLock {
    ...
    _buildPatchConfig(kubeConfig) {
        ...
        return k8s.createConfiguration({
            ...
            authMethods: { default: { applySecurityAuthentication: async (req) => {
    ...
    async _lock() {
        ...
        let lease;
        try {
            lease = await k8sApi.readNamespacedLease( { name: this.leaseName, namespace: this.namespace });
        }catch (e) {
            if(e?.code === 404 && this.createLeaseIfNotExist) {
                lease = await k8sApi.createNamespacedLease( { namespace: this.namespace, body: {
                    metadata: {
                        name: this.leaseName,
                        labels: this.labels
                    },
        ...
        if(new Date(lease.spec.renewTime || 0 ) < new Date() || lease.spec.holderIdentity === this.lockLeaserId) {
            ...
            try {
                ...
                await k8sApi.patchNamespacedLease( { name: this.leaseName, namespace: this.namespace, body }, this._buildPatchConfig(this.kubeConfig));
            }catch (e) {
                if(e?.code === 409) {
                    this.isLocking = false;
                    return false;
